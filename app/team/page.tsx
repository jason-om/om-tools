...
const employees:Employee[]=[
  ...
  {name:"Theo Martin",initials:"TM",title:"Growth Strategist",department:"Growth",timeZone:"America/Detroit",timeZoneLabel:"Eastern Time (ET)",slackUserId:"UOMTHEO",tone:"coral",omEvents:[{time:"9:00 AM",title:"OM planning huddle"}]},
  ...
  {name:"Leah Kim",initials:"LK",title:"Content Designer",department:"Creative",timeZone:"America/New_York",timeZoneLabel:"Eastern Time (ET)",slackUserId:"UOMLEAH",tone:"pink",omEvents:[{time:"10:30 AM",title:"Sonterra creative review"}]},
];


function loadLevel(events:OmEvent[]):LoadLevel{return events.length>=4?"Busy":events.length>=2?"Medium":"Light"}
...
function glanceCopy(events:OmEvent[]):string{
  if(events.length>=4) return "Packed with OM commitments today – recommend scheduling for another day. But won’t hurt to ask :)";
  if(events.length===3) return "A few OM touchpoints today – there still look to be openings around them. Won’t hurt to ask.";
  if(events.length===2) return "A couple of OM touchpoints today – potential openings around them. Won’t hurt to ask.";
...
function OmSchedule({employee}:{employee:Employee}){const level=loadLevel(employee.omEvents);return <div className="om-schedule"><p className={`schedule-note ${loadTone[level]}`}><strong>OM day at a glance<em>{level}</em></strong><span>{glanceCopy(employee.omEvents)}</span></p><header><div><CalendarDays size={14}/><b>Today’s OM calendar</b></div><span>{employee.omEvents.length} {employee.omEvents.length===1?"event":"events"}</span></header>{<div className="om-events">{employee.omEvents.map(event=><div key={`${event.time}-${event.title}`}><time>{event.time}</time><span>{event.title}</span></div>)}</div>}</div>}
...
function EmployeeList({items,now}:{items:Employee[];now:Date}){return <section className="employee-list" aria-label="All OM employees list"><header><span>Employee</span><span>Time zones</span><span>Today’s OM calendar</span><span>Schedule context</span><span>Contact</span></header>{items.map(employee=>{const level=loadLevel(employee.omEvents);return <article key={employee.name}><div><span className={`employee-avatar ${employee.tone}`}>{employee.initials}</span><span><b>{employee.name}</b><small>{employee.title} · {employee.department}</small></span></div><div className="list-times"><span><b suppressHydrationWarning>{displayTime(now,employee.timeZone)}</b> local</span><small>{employee.timeZoneLabel}</small><span><b suppressHydrationWarning>{displayTime(now,OM_TIME_ZONE)}</b> OM</span></div><div className="list-events">{employee.omEvents.map(event=><span key={`${event.time}-${event.title}`}><time>{event.time}</time>{event.title}</span>)}</div><p className="list-context"><em className={`load-pill ${loadTone[level]}`}>{level}</em>{glanceCopy(employee.omEvents)}</p><a className="slack-dm" href={slackUrl(employee.slackUserId)} target="_blank" rel="noopener noreferrer"><MessageSquare size={15}/>Slack DM</a></article>})}</section>}
...
export default function TeamPage(){
  ...
  const groups=useMemo(()=>{const grouped=new Map<string,Employee[]>();visibleEmployees.forEach(employee=>{const key=group==="department"?employee.department:loadLevel(employee.omEvents);grouped.set(key,[...(grouped.get(key)||[]),employee])});return [...grouped.entries()]},[group,visibleEmployees]);
